...
command_def_t bossd_cmd_table[] = {
   ...
   {name: "startin", type: CMD_GROUPMAN1,
    fun: {groupman1: ACTION(startin_proc)},
    description: "Start processes in specified terminal"},
   {name: "show", type: CMD_GROUPMAN,
    fun: {groupman: ACTION(NULL)},
    description: "Start process in current terminal"},
   ...
   {name: "explode", type: CMD_NOARG,
    fun: {noarg: ACTION(restart_supervisor)},
    description: "Restart supervisor in place and check processes"},
   {name: "reload", type: CMD_NOARG,
    fun: {noarg: ACTION(restart_supervisor)},
    description: "Restart supervisor in place and check processes"},
   {name: "reopenlog", type: CMD_NOARG,
    fun: {noarg: ACTION(reopenlogs)},
    description: "Reopen log file in case it's rotated by external tool"},
   {name: "rotatelog", type: CMD_NOARG,
    fun: {noarg: ACTION(rotatelogs)},
